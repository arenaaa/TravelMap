...
public class UserDao {
	...
	/*
	 * 파라미터를 UserVO로 하니까 사용하는 쪽에서 불편함.
	 * 아래처럼 id와 password를 각각 전달받게 하면 쓰는 쪽이
	 * 좀 더 편함!
	 * 
	 */
	public UserVO Login( String userId, String password ){
		...
		// 익명 클래스라고 합니다.
		/*
		 * 인스턴스를 만들때 항상 클래스 파일이 있었습니다.
		 * 그런데 어떤 경우에는 클래스를 별도로 만들어서 쓰는게 귀찮음!!!
		 * 그냥 바로 하나 만들어서 쓰고 싶다!!
		 */
//		RowMapper<UserVO> mapper = new UserMapper();
		...
		List<UserVO> users = template.query(query, new Object[]{userId, password}, new RowMapper<UserVO>(){
			 ...
			 * while ( rs.next() ) {
			 *    UserVO user = mapper.mapRow ( rs, idx );
			 *    users.add ( user );
			 *    idx ++;
			 * }
			 * 
